pipeline {
    ...
    stages {
        ...
        stage('Deploy Docker Image to Kubernetes with Ansible') {
            steps {
                sh """
                        ansible-playbook -i ansible/inventory.ini ansible/ping_test.yml -u jenkins-master --private-key ~/.ssh/authorized_keys
                        ansible-playbook -i ansible/inventory.ini ansible/deploy_k8s.yml -u jenkins-master --private-key ~/.ssh/authorized_keys
                    """
            }
        }
        stage('Archive WAR') {
